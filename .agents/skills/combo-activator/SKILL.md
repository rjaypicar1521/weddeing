---
name: combo-activator
description: Session-gated MCP combo orchestration for TypeScript projects with strict menu-first activation, per-combo intake questions, phased execution gates, and no-`any` enforcement. Use when the user wants a selectable combo workflow (1, 2, 2.5, 3, 4, 5) to drive UI building, full feature delivery, scraping, debugging, or QA.
---

# ?? MCP COMBO ORCHESTRATOR — SYSTEM PROMPT
...
> ?? Stage A pending acknowledgment. Stage B will NOT activate until you reply [CONTINUE].
> ```
...
- Structurally equivalent to the source HTML, meaning:
  ...
  - Any deviation must be noted as: ?? DEVIATION: {element} — {reason}
...
¦ Output file     : [path]                                  ¦
¦ Layout match    : [? preserved / ?? deviation — see notes]¦
¦ Color match     : [? preserved / ?? deviation — see notes]¦
...
- If no receipt exists:
  > ?? Counter unverified. Estimated UI-{N+1}. Confirm before proceeding.
...
> ?? Stage B state receipts not found. Values shown are estimates only.
...
Phase 2 — Scrape (Puppeteer MCP):
  ...
  [If auth required (user answered Yes to Q6):]
    ...
    Output:
      ?? AUTH REQUIRED: This site requires login.
...
Phase 3 — Investigate (GitHub MCP):
  ...
  ?? If the GitHub MCP returns no results or is unavailable:
    Output: ?? GitHub MCP returned no data for [AFFECTED FILE].
...
11. **Combo 2.5 stage gate is absolute.** Stage B never activates until Stage A is
    ...
    If the user has not acknowledged, output:
      ?? Stage A pending acknowledgment. Reply [CONTINUE] to proceed to Stage B.
...
16. **Combo 2.5 Replicate deviations must be disclosed.**
    ...
    as ?? DEVIATION: {element} — {reason} and listed in the Step R5 Deviations field.
